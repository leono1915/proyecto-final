...
function newRowTable() {
	...
	var nombreFinal = nombrey[nombrex].text;
	var medidaFinal = medidaY[medidaX].text;
	var espesorFinal = espesorY[espesorX].text;
	...
	$.ajax({
		url: 'calculos.php',
		type: 'POST',
		data: {  nombreFinal,  medidaFinal, espesorFinal },
		success: function (respuesta) {
...
function imprimir() {
    ...
    var porcentajeDescuento=document.getElementById('opcionAdescontar').value;
	var opcion=document.getElementById('opcionAimprimir').checked;
	console.log(porcentajeDescuento)
	...
	switch(opcion){
	case true: window.open("plantillasiniva.php?nombreCliente=" + nombreCliente + "&descuento="+ descuento +"&porcentajeDescuento="+porcentajeDescuento +"");
	
	break;
	case false: window.open("plantilla.php?nombreCliente=" + nombreCliente + "&descuento="+ descuento +"&porcentajeDescuento="+porcentajeDescuento +""); 
	break;
	}
...
function tablaParametros(cantidad,descripcion,precioUnitario,subtotal,iva,total,accion,id,cantidadDescontar,metros,tramos){
	...
	$.ajax({
		...
		success:function(respuesta){
			//alert(respuesta);
				console.log("me ejecuto y soy tabla de parametros");
			listarTablaTemporal();
		}
	})
